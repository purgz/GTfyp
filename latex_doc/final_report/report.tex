...
This is where I will introduce replicator equations and standard work for rock paper scissors\cite{PhysRevLett.95.238701}
...
\subsection{\label{sec:rps_replicator} Replicator dynamics}
...
Introducing a loner strategy with frequency $q = 1 - x - y - z$, we define the snowdrift coexistence measure
\[
H_{\mathrm{SD}} = - q (1-q).
\]
...
These equations are not correct form just yet - need to manually change.

\subsection{Four-Strategy Coexistence Drift}

To capture full four-strategy coexistence, we consider the measure
\[
H_4 = - x y z q ,
\]
...
\sum_{k=1}^{N-i-j}
\Big[
k(1-i-j-k)(j-i)(T^{RP}-T^{PR})
\nonumber \\
&\quad
+ j(1-i-j-k)(k-i)(T^{RS}-T^{SR})
+ i(1-i-j-k)(j-k)(T^{SP}-T^{PS})
\nonumber \\
&\quad
+ jk(-2i-j-k)(T^{R+}-T^{+R})
+ ik(-2j-i-k)(T^{P+}-T^{+P})

...
When $a = 0$, standard RPS ties, this can be seen in the $2 \times 2$ snowdrift context as the following
payoff matrix:
\begin{align} \nonumber
\label{CompositeGame}
\begin{bmatrix}
...
\right\} \nonumber\\
\end{align}
these eigenvalues describe the dynamics in Fig.~\ref{fig:arps_spiral_in}, similarly to the neutral cycles
...
components once again indicate the spiralling nature of these trajectories.
...
of $N \rightarrow \infty$ is defined as the paiwise difference form of the 
replicator dynamics:
\begin{align}
\dot{x}_{FE} &= x \Big[
...
\begin{align}
N^\prime_{SD} &= \frac{1}{280N^2 \Delta \pi_{max}} \Bigg[42 \Delta \pi_{\text{max}} \nonumber \\
&+ 2w \gamma (7 - 6N) \nonumber\\
...
in the RPS plane, when $N^\prime_{RPS} < N < N^\prime_{SD}$ the oppsite is true and the reversal takes place in the 

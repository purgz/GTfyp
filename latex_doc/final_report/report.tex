...
\begin{abstract}
This project investigates the evolutionary dynamics of a composite game formed by combining 
a cyclical three strategy Rock-Paper-Scissors (RPS) game with a two strategy Snowdrift (SD) game.
The resulting dynamics are analysed in infinite and finite populations using techniques in evolutionary game theory.

A payoff structure is formed and cast into replicator equations, fixed points and stability are analysed and 
the deterministic dynamics of the 4 strategy system are observed combining cyclical behaviour and 
Snowdrift cooperate/defect tradeoff dynamics.

Finite population effects are studied through the use of different microscoping interaction processes, 
such as the Moran process and the local update process. Analytical calculations
...
dynamics in the 4-simplex. The phenomenon of drift reversal is also examined, where
an attracting fixed point loses stability below a critical population size in the 
sub games and the combined game.

The results demonstrate the effects of game parameters, interaction processes, 
strength of selection and population sizes impact the dynamics of the system and 
when a drift reversal will occur, in particular comparing different cases where the 
critical population size differs between the sub games. A set of tools
has been developed to calculate trajectories, run agent based simulations and 
visualize the dynamics in the combined game (*project README for more information about 
code tools).
...
RPS demonstrates cyclic dominance and SD demonstrates coexistence in a population 
based on a cooperation/defection tradeoff in the context of a shared cost.
The resulting game contains both cyclical and coexistence stability leading
to some interesting dynamics, invasion into the RPS game by a fourth strategy,
particularly in finite populations where multiple 
drift reversals can occur.
...
\begin{itemize}
  \item Construction of composite RPS-Snowdrift payoff structure
  \item Stability analysis in infinite populations
  \item Derivations of drift observables
  \item Simulation of stochastic evolutionary processes
  \item Numerical investigation of drift reversal in the 4 strategy game
\end{itemize}
...
These are the deterministic payoff difference replicator equations from 
...
alongide solution to deterministic replicator equation for each process from the 
...
these values for each interaction process in order to plot the correct deterministic adjusted dynamics.
...
[Graph here compariosn against the weak selection limit]




Calculate the RPS weak selection limit.
...
A similar process can be done for the RPS drift observable leading eventually to:
...
[Add trajectory comparison for LU, MO, and FERMI process to show the difference]

[Add Delta H graph for LU, MO and fermi process for one of the 
drift observables show that interaction process changes dynamics.]

[Time series of 4th strategy for the 3 processes only, dont include RPS]

[Changing W over time changing critical N]

[Average traejctory time series demonstrating drift reversal.]
...
\begin{figure}
  ...
  \caption{RPS reversal $\beta=0.24, \gamma=0.4, c=-0.8$ cleaner disk case, clamped the starting values where $q > 0.1 < 0.9$ to prevent RPS random extinctions for a clearer image.

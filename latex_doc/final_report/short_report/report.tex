...
over time $\dot{x}, \dot{y}$ it follows that
$\dot{z} = 1 - \dot{x} - \dot{y}$. (see long expressions in appendix)
...
considering the case where $N$ is infinitely large give the three equations:
$\dot{x}, \dot{y}, \dot{z}$ and $\dot{q} = 1-\dot{x} - \dot{y} - \dot{z}$.
(please see these in the appendix, they are not included due to page limit)
...
that can be converted into the following continuous form where
...
can be defined, similarly to the RPS case, by the following integral.
...
The expected value of $\Delta H_4$ can be defined like the previous two and converted from 
sum to integral eventually resulting in the following expression:



\begin{align}
\langle \Delta H_{4} \rangle = &\frac{6}{N} \int_{0}^{1}dx \int_{0}^{1-x} dy \int_{0}^{1-x-y}dz \Big[ \nonumber \\
zq(y - x)&(T^{RP} - T^{PR}) + yq(z - x)(T^{RS} - T^{SR}) \nonumber \\
+xq(y-z)&(T^{SP} - T^{PS})+ yz(q-x)(T^{R+} - T^{+R}) \nonumber \\
+ xz(q-y)&(T^{P+} - T^{+P}) + xy(q-z)(T^{S+} - T^{+S})\Big] \nonumber \\
&+ \frac{6}{N^2} \int_{0}^{1}dx \int_{0}^{1-x} dy \int_{0}^{1-x-y}dz\Big[ \nonumber \\
&zq(T^{RP} + T^{PR}) + yq(T^{RS} + T^{SR}) \nonumber \\
&+ xq(T^{SP} + T^{PS}) + yz(T^{R+} + T^{+R}) \nonumber \\
&+ xz(T^{P+} + T^{+P}) + xy(T^{S+} + T^{+S})\Big]
\end{align}
...
By expanding the difference terms and replacing these in the 
original integral (equation \ref{eq:delta_h_sd}), the integral can now be solved exactly
leading to:

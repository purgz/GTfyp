...
equations that can be formulated for RPS (see appendix RPS). 
...
\begin{figure*}
\centering
\includegraphics[scale=0.35]{Figures/arps_time_series_standard.pdf}
...
\begin{figure*}
\centering
\includegraphics[scale=0.55]{Figures/spiral_in_time_series.pdf}
...
Performing a second order Taylor expansion on the probabilites and 
transition probabilites the following expression is eventually obtained:
...
$b(x)$ which dissapears as $N \rightarrow \infty$.
...
agent based simulation by calculating $\langle \Delta H \rangle = \langle \Delta H(t+1) - H(t) \rangle$ 
...
large N Nash equilibrium stable case where $N >> N_c$ and chaotic low N case (drift in all directions)
 ...
 random point in the simplex. View the table of values used for this case graph in the appendix}
\end{figure*}

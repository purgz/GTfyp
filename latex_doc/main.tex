...
Finite population, $i$ rock players, $j$ paper players, $k$ scissors players, and $N - i - j - k$ $+$ players.

The average payoff is $\langle \pi \rangle = \frac{i}{N} \pi_R + \frac{j}{N} \pi_P + \frac{k}{N} \pi_S + \frac{N - i - j -k}{N} \pi_+$
...
Example output from python for $T^{R \rightarrow +}$, with x, y, z, 1-x-y-z,
and parameters a,b,c for rps, $\gamma$ and $a + \beta$ version of the 4x4 matrix.
$- \frac{0.5 x \left(w \left(a + \beta\right) \left(x + y + z\right) - w + 1\right) \left(x + y + z - 1\right)}{w \left(x \left(a x + b z + c y - \gamma \left(x + y + z - 1\right)\right) + y \left(a y + b x + c z - \gamma \left(x + y + z - 1\right)\right) + z \left(a z + b y + c x - \gamma \left(x + y + z - 1\right)\right) - \left(a + \beta\right) \left(x + y + z\right) \left(x + y + z - 1\right)\right) - w + 1}$
...
\begin{subfigure}{.5\textwidth}
  ...
  \caption{$N = 850$}
\end{subfigure}%
\caption{Strong attracting RPS with matrix in figure, reversal seen in SD but not in RPS, large population size tends to the deterministic case.}
\end{figure}

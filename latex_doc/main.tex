...
\begin{equation}
  \begin{aligned}
    P^{\tau + 1}(i) - P^\tau(i) &= (P-\frac{1}{N}P' + \frac{1}{2N^2}P'')(T^{PR} - \frac{1}{N}T^{PR'} + \frac{1}{2N^2}T^{PR''}) - PT^{PR} \\
...
\noindent We then have for rock (i) $a(x) = T^{PR} + T^{SR} + T^{+R} - T^{RP} - T^{RS} - T^{R+}$ and $b^2(x) = \frac{T^{PR}+T^{SR}+T^{+R} + T^{RP} +T^{RS}+T^{R+}}{N}$. In the same way we can find $a(x), b^2(x)$ for the other strategies. 

\par \noindent
For paper (j) we get $a(x) = $ and $b^2(x) = $ \\
For scissors (k) we get $a(x) = $ and $b^2(x) = $ \\
From these we can form the fokker planck equations for each strategies population $(x \in \{i,j,k\})$:

\begin{equation}
  \frac{1}{N}\Big(-\frac{\partial}{\partial x}\big(P(x) a(x)\big) + \frac{1}{2} \frac{\partial^2}{\partial x^2}\big(P(x) b^2(x)\big)\Big)
\end{equation}
...
\begin{equation}
  \dot{x} = a(x) + b(x)\xi
\end{equation}
$a(x)$ is our deterministic replicator equation as $N \rightarrow \inf$ noise dissapears.

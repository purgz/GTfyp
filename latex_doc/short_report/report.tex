...
in the infinite-population case~\cite{evolution_pop_dynamics}, this project extends this analysis to
...
results in distinct dynamics illustrated in section~\ref{sec:drift_measures}.
...
defines transition probabilities for individuals hopping from one strategy to another. Section 
...
\caption{\label{fig:rps_standard} Standard zero-sum RPS game ($a=0, b=1, c=-1$), 
$w = 0.3$. Analytical trajectory (blue), large $N$ simulation ($N=500000$, green) 
and small-$N$ simulation ($N=2000$, orange). The large $N$ trajectory closely follows
the replicator dynamics (orbits at fixed distance from center). The small $N$ trajectory 
deviates due to stochastic fluctuations but still orbiting the interior fixed point 
...
trajectories oscillate at a fixed distance from the Nash equilibrium $(1/3, 1/3, 1/3)$ (Fig.~\ref{fig:rps_standard}). 
When $c < -1$ the system spirals outwards from the equilibrium and will be absorbed by the boundaries and
finally when $c > -1$ the system will spiral inwards (Fig.~\ref{fig:rps_sink}), where cooperation between the three strategies is
preferred, as shown via stability analysis for the system of replicator 
equations that can be formulated for RPS (full derivation in appendix). 
It has previously been shown that a drift reversal effect occurs when $c > -1$ where a critical 
...
\caption{\label{fig:rps_sink} RPS game with $c=-0.8$, $w=0.3$. The blue line corresponds to a numerical
solution of the adjusted replicator dynamics for the Moran process, and the green line is a Moran process
agent based simulation with $N = 500$. 
Negative real part of eigenvalues (see appendix) produces a spiral sink for the replicator systems, and 
trajectories spiral inwards to the fixed point $\left(\frac{1}{3},\frac{1}{3},\frac{1}{3}\right)$.
At $N=500$, simulation tracks the deterministic trajectory with small deviations due to finite population size
noise.}
\end{figure}
...
and is an example of an anti-coordination game where there are two pure strategy Nash equilibria when 
...
There are two pure strategy Nash equilibria where the players select opposite strategies, 
and a mixed strategy Nash equilibrium.
The interior mixed strategy Nash equilibrium for this payoff matrix occurs at the mixed strategy $q^* = \frac{b-2c}{b-c}$ where 
...
\caption{\label{snowdrift_trajectory} Snowdrift game population fraction time series for Local update (LU) and Moran process (MO), 
alongside solution to deterministic replicator equation for each process from the 
adjusted dynamics derived in section.~\ref{sec:fokker-planck}. $N = 10000, w=0.9, c=1, b=3$,
Simulated large $N$ trajectories closely follow corresponding adjusted replicator dynamics. 
Initial population 99\% cooperators.
Both processes converge to the stable mixed strategy Nash equilibrium  $q^* = \frac{1}{2}$} 
\end{figure}
...
Let $x,y,z, q$ denote the fraction of the population playing rock, paper, scissors, and $+$ respectively, 
$q$ can be rewritten as $q = 1-x-y-z$.

The standard replicator dynamics \cite{evolution_pop_dynamics} 
considering the case where $N$ is infinitely large result in three 
replicator equations $\dot{x}, \dot{y}, \dot{z}$ describing the time
evolution of each strategies population over time (full equations in appendix). 


Defining $F = \dot{x}, G = \dot{y}, P = \dot{z}$ the Jacobian for this system of replicator equations is:
...
Considering the zero sum RPS instance of this game where
$a = 0, b=1, c= -1$, and $\gamma = 0.2, \beta = 0.1$, 
...
before for standard rock-paper-scissors seen in Fig.~\ref{fig:rps_standard}.
...
$\left(\frac{2}{9},\frac{2}{9},\frac{2}{9},\frac{1}{3}\right)$. The numerical solution is compared 
against a simulated finite population trajectory in Fig.~\ref{fig:arps_standard_simplex} and 
Fig.~\ref{fig:arps_standard_time_series}.


\begin{figure}
\centering
\includegraphics[scale=0.43]{Figures/arps_standard_simplex.pdf}
\caption{\label{fig:arps_standard_simplex} Simplex trajectory for the combined game with 
standard RPS ($a=0, b=1, c=-1$), $\gamma=0.2$, $\beta=0.1$, $w=0.45$, $N=50000$.
Numerical solution (blue) and Moran process simulation (green), with simulation iterations normalized by $N$ to 
match the ODE time steps (150 steps). The trajectory converges to the RPS plane (stable in the SD axis, 
eigenvalue $-\frac{1}{15}$), and then forms periodic orbits at a fixed distance from the interior fixed point 
$(2/9, 2/9,2/9,1/3)$ (purely imaginary eigenvalues). Initial populations used [0.5,0.2,0.2,0.1], for 
R,P,S,+.}
\end{figure}

\begin{figure*}
\centering
\includegraphics[scale=0.30]{Figures/arps_time_series_standard.pdf}
\caption{\label{fig:arps_standard_time_series} Time series of strategy frequencies for the same trajectory in 
Fig.~\ref{fig:arps_standard_simplex}. Dashed line are numerical solutions of the Moran adjusted replicator dynamics
($a(x)$ from Fokker-Planck derivation), solid lines are simulated results. RPS oscillations and the 
convergence of $+$ frequencies (red line) are both visible.}
\end{figure*}
...
\caption{\label{fig:arps_spiral_in} 
Simplex trajectory for the combined game with $c=-0.8$, $\gamma=0.2$, $\beta=0.1$, $w=0.45$, $N=50000$.
The negative real parts of all three eigenvalues produce a stable spiral sink
and trajectories converge to $\left( \frac{2}{7},\frac{2}{7},\frac{2}{7}, \frac{1}{7} \right)$.
Numerical solution to replicator equation (blue) and Moran process simulation (green) with strong agreement 
at the large population $N = 50000$.}
\end{figure}

\begin{figure*}
\centering
\includegraphics[scale=0.50]{Figures/spiral_in_time_series.pdf}
\caption{\label{fig:arps_spiral_in_time_series}
Time series of strategy frequencies for same data as Fig.~\ref{fig:arps_spiral_in}.
Dashed lines correspond to Moran replicator dynamics, solid lines are Moran process simulations.
The amplitude of RPS oscillations decays over time as the trajectories converge to the stable interior fixed point. 
Small fluctuations due to finite population but agreement with deterministic dynamics.}
\end{figure*}
...
be selected and reproduce according to the different schemes definition
...
A generalized form of the Moran process in terms of transition probabilities between states in the 
...
Transitions within the RPS game are of the form:
...
strategies can be formed by defining appropriate reproductive functions $\phi$ between 
two strategies for each of the processes (see appendix).
...
deterministic replicator dynamics, and stochastic noise from finite $N$.
...
This is composed of transition probabilities into and out of the 
strategy R and describes the time evolution of the system 
jumping between discrete states (the count of $i$ rock individuals).


Performing a second order Taylor expansion on the probabilities and 
transition probabilities the following expression is eventually obtained:
...
For full derivation of the Fokker-Planck equation for this system and for 
$a(y), b(y), a(z), b(z)$,
see the appendix.
...
scaled by a constant term containing $w$,
...
adjusted dynamics in which individuals fitness is evaluated against the population average payoff as opposed to 
local interactions for example in the local update process.
...
\dot{x} = a(x) + b(x) \xi
\end{equation}
which is a combination of the deterministic dynamics ($a(x)$) and a noise term 
$b(x)$ which disappears as $N \rightarrow \infty$.
...
$w$ which was not considered in infinite $N$ scenarios.
...
drift observables, and exploring low $N$ cases.
...
agent based simulation by calculating $\langle \Delta H \rangle = \langle H(t+1) - H(t) \rangle$ 
for a single update step and then averaging over many repeated simulations (see Fig.~\ref{fig:n_c_against_w}), 
however for all the interaction processes the terms can be 
formulated as an integral and solved numerically and in some cases e.g. local update, a 
...
indicate whether the system drifts towards or away from the interior equilibrium \cite{drift_reversal_paper}.



Drift reversal is defined as the change in sign of $\dot{H}$ under
variation of parameters below a critical population size.
...
written as a sum over the simplex for each of the 
subgames observable $H_{SD}, H_{RPS}, H_4$:
...
Where transition probabilities $T^{s \rightarrow s'}$ must be defined for each of the 
...
can be defined by a sum as follows:
...
This can be approximated in the continuous limit by the following integral where $x = i/N, y=j/N, z=k/N, q = 1-x-y-z$:
...
$H_{SD} - H_{SD}' = H_{SD} - H_{SD} = 0$ for all transitions
($R \rightarrow P, P \rightarrow S, S \rightarrow R$,
and their reverses) within the RPS subgame. 
Since transitions purely within the RPS subgame do not change the value
of $H_{SD}$, this exact term depends only on transitions into and out of 
the fourth strategy ($+$). 
...
H_{\mathrm{RPS}} = - x y z
\end{align}
which is a measure of the distance of a population from the internal fixed point of RPS.
$\langle \Delta H_{RPS} \rangle$ is approximated by a sum and converted into the following continuous form where
...
The expected value of $\Delta H_4$ in the continuous form is:
\begin{widetext}
\begin{equation}
\begin{aligned}
...
The sign of $\langle \Delta H_4 \rangle$ determines whether stochastic dynamics favors full four-strategy coexistence or boundary attraction.
...
\caption{\label{fig:weak_selection_comparison}
$\langle \Delta H_{\mathrm{SD}} \rangle_{LU} \cdot N^2$ as a function of $N$ for the local update process
at five values of $w$, with $a = 0, b = 1, c=-0.2, \gamma = 0.14, \beta = 0.3$.
A drift reversal for these parameters occurs in all the cases where $w > 0$, and as $w \rightarrow 0$, the 
lines approach the weak selection limit (neutral evolution) of $\langle \Delta H_{\mathrm{SD}} \rangle_{LU} \cdot N^2 = \frac{3}{20}$.
Intersections with $\langle \Delta H_{\mathrm{SD}} \rangle_{LU} \cdot N^2$ indicate a drift reversal, and the 
$N$ value this occurs at is the critical population size.}
\end{figure}
...
$\langle \Delta H_{RPS} \rangle$_{w \rightarrow 0} &= \frac{2}{N^2} \int_{0}^{1}dx\int_{0}^{1-x}dy\int_{0}^{1-x-y}dz\left(3xyz\right) \nonumber\\
&= \frac{1}{120N^2}
\end{align}
...
altered to adjust the dynamics of the RPS component of the game. $\langle \Delta H_{SD} \rangle$ is composed of differences of the form 
$T^{R+}-T^{+R}$. Expanding these differences for the local update process:
...
The other transition probability differences are symmetrical to the above with the 
population count and payoff switched to their respective values.

The second component of $\langle \Delta H_{SD} \rangle$ are the terms scaled by $1/N^2$, $T^{+R} + T^{+P} + T^{+S}$.

This expression eventually simplifies to the following in terms of $w, \Delta \pi_{max}$ and the payoff
matrix parameters:
...
results in an exact term for the critical population size by setting this to 0:
...
The difference terms $T^{RP} - T^{PR}$ for the local update transition probabilities are of the form:
...
After substituting in the expanded forms of the transition probability differences, $\langle \Delta H_{RPS} \rangle_{LU}$
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.45\textwidth]{Figures/critical_n_analytical.png}
    \caption{\label{fig:n_c_against_w}
    Critical population sizes $N_c$ as a function of selection strength $w$ for the SD observable $H_{SD}$. 
    Moran process simulations with  ($\gamma=0.2$, $\beta=0.1$, standard RPS parameters)
    Points are simulated estimate of $N_c$ by approximating $\langle \Delta H \rangle$ over a single update step, 
    averaged over $10^8$ realizations. Curve is analytical prediction when setting equation~\ref{eq:delta_h_sd}
    equal to 0.
    Close agreement, and drift reversal clear in both analytical result and simulated results.}
\end{figure}


\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{Figures/proces_drift_time_series.pdf}
    \caption{\label{fig:drift_time_series} 
    Time series of the fraction of $+$ strategy for three interaction processes:
    Moran (red), local update (blue), and local Fermi (green).
    Dashed lines are the deterministic replicator equation solutions for each process.
    Large $N$ ($N=100000$) simulations closely follow the 
    deterministic trajectory with small noise deviations.
    Low $N$ ($N=200$) simulations, 
    averaged over 1000 realisations, exhibit drift reversal. Trajectories move away from 
    the deterministic fixed point and the $+$ fraction converges towards zero, as the population ends up playing
    purely RPS ($q \rightarrow 0$), opposite to the replicator prediction.}
\end{figure}
...
Each of these cases result in significantly 
...
drift expected from the replicator deterministic dynamics, e.g. the 
...
\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{Figures/column_case_delta_H.eps}
    \caption{\label{fig:sd_reversal_case}
     $\langle \Delta H \rangle \cdot N^2$ against 
    $N$ for the local update process showing the SD reversal case, for each drift measure
    $\langle \Delta H_{SD} \rangle,
    \langle \Delta H_{RPS} \rangle, \text{and} \langle \Delta H_+ \rangle$ with
    $\beta=0.3$, $\gamma=0.14$, $c=-0.2$, $w=0.45$.
    Since $N^\prime_{RPS} < N^\prime_{SD}$, the sign of the SD observable reverses first at a higher $N$.
    Since $N > N^\prime_{RPS}$ the RPS game converges to its interior fixed point while the fourth strategy drifts away 
    since $N <  N^\prime_{SD}$ producing the column shaped trajectory cloud in Fig.~\ref{drift_cases_fig}.}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=0.45\textwidth]{Figures/disk_clean_deltaH.eps}
  \caption{
    \label{fig:rps_reversal_case}
  $\langle \Delta H \rangle \cdot N^2$ against 
  $N$ for the local update process showing the RPS reversal case with $\beta=0.24$, $\gamma=0.4$, $c=-0.8$, $w=0.45$.
  The exact opposite of the SD reversal case in Fig.~\ref{fig:sd_reversal_case}, where RPS drifts at the larger population size and 
  SD remains stable. For $N^\prime_{SD} < N < N^\prime_{RPS}$ trajectories move towards the RPS boundaries, away from the interior fixed point, 
  but remain stable in the SD direction resulting in the disk shaped trajectory cloud in Fig.~\ref{drift_cases_fig}.}
\end{figure}
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{\label{drift_cases_fig} 
Summary of the four drift reversal cases
(table of parameter values and critical population sizes provided in the appendix).
The dashed diagonal marks where $N_{SD} = N_{RPS}$, points below this line have $N_{RPS} > N_{SD}$, 
and the reverse for points above this line. The horizontal and vertical lines at $10^0$ indicate where the ratio $N / N_{RPS} = 1$ or 
$N / N_{SD} = 1$ categorising the cases, so below these lines (where the ratio is less than 1), the population size is less than the critical 
population size for drift reversal.
The lower left region indicates the low population size case where $N << N_{RPS}, N << N_{SD}$ and there is a double drift reversal.
The upper left region indicates the case where $N_{RPS} < N < N_{SD}$ and there is a drift reversal in the SD axis but not the RPS plane.
The lower right region indicates the case where $N_{SD} < N < N_{RPS}$ and there is a drift reversal in RPS but not SD.
The upper right region is the large population case where the trajectories follow the deterministic replicator dynamics predictions, which is 
convergence to the interior fixed point for each of these games ($N >> N_{RPS}, N >> N_{SD}$).}
\end{figure*}
...
as the strength of selection $w \rightarrow 0$. Deterministic adjusted replicator dynamics as 
...
get absorbed through pure random noise. The mutation extension has also been considered in
